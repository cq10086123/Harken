...
- 一条由多条白色细线扭成的**声波带**横贯画面，中部展开、**两端收束成一个尖点**；
- 无任何多余装饰，扁平、双色。

声波带的做法是给每条线加一层 `sin(πt)^p` 包络（两端归零），再让各条线相位错开：
包络保证所有线在左右两端汇聚成尖点，相位差让它们在中部自然扭开成带。
这样「一条线」和「一整条带」是同一个几何的自然结果，不需要拼接图形。

比例：

| 指标 | 取值 | 说明 |
| :--- | :--- | :--- |
| 带宽（振幅） | 画布 21.5% | 决定整条带的高度 |
| 振荡次数 | 3 | 中部三个波峰；少了像一条胖 M，多了变装饰花纹 |
| 相位错开 | 0.44 | 决定扭带展开的宽度 |
| 线宽 | 画布 0.18%（256px 以上） | 中间调，靠线数堆出体积感 |

### 按尺寸分级

细线在 48px 以下会**相位混叠**，看起来「峰数都变了」。因此生成器按输出尺寸换档，
尺寸越小线越少、线越粗、振荡次数越少——任何一个尺寸都读得出是一条声波：
...
图标底色本身是深色渐变，在浅色与深色系统下观感一致，因此**只有一套资源**，
...
| 位置 | 处理 |
| :--- | :--- |
| Android 方形图标 | 满幅渐变底 + 声波带，由系统套蒙版 |
| Android 自适应图标 | 渐变底用 `drawable/ic_launcher_background.xml`（`shape` + `gradient`），前景为缩到中心安全区（62%）的白标透明底；另有单色层供 Android 13+ 主题图标 |
| Android 开屏 | 整枚图标（渐变底 + 圆角），明暗开屏背景上都成立 |
...
> monochrome 层）。修改图标请改 `scripts/generate_icons.py` 的 `WAVE` 参数后重新运行。
